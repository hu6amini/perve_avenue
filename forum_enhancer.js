...
// Updated to be compatible with timestamp script processing
class PostModernizer {
    ...
    #setupObserverCallbacks() {
        ...
        this.#debouncedObserverId = globalThis.forumObserver.registerDebounced({
            ...
            selector: '.post, .st-emoji, .title2.bottom, div[align="center"]:has(.quote_top), div.spoiler[align="center"], div[align="center"]:has(.code_top), .u-dt, .when, time[class*="dt"], .title2.top .lt.Sub',
            delay: 100,
            priority: 'normal',
    ...
    #handlePostTransformation(node) {
        ...
        const needsTransformation = node.matches('.post') ||
            ...
            node.querySelector('.title2.top') ||
            node.querySelector('.title2.top .u-dt, .title2.top .timeago, .title2.top .when') ||
    ...
    #transformPostElements() {
        ...
        posts.forEach((post, index) => {
            ...
            if (title2Top) {
                ...
                // Handle timestamp-processed elements
                const timestampElement = title2Top.querySelector('.u-dt, .timeago, .when, time');
                const ltSub = title2Top.querySelector('.lt.Sub');
                
                if (tdWrapper) {
                    const title2TopClone = title2Top.cloneNode(true);
                    
                    // Preserve timestamp elements - don't remove them
                    ...
                    // Ensure timestamp elements are preserved
                    if (ltSub) {
                        const ltSubClone = ltSub.cloneNode(true);
                        // Check for already processed timestamps
                        const existingTimeElement = ltSubClone.querySelector('time');
                        if (!existingTimeElement) {
                            // If no time element, preserve the original structure
                            // The timestamp script will handle it later
                        }
                    }
            ...
            // Restore timestamp elements after transformation
            this.#restoreTimestampElements(post);
    ...
    #transformSearchPostElements() {
        ...
        posts.forEach((post, index) => {
            ...
            // Preserve timestamp elements in search posts
            this.#restoreTimestampElements(newPost);
        });
    }
...
#preserveMediaDimensions(element) {
    ...
    #preserveTimestampElements(element) {
        // Find and preserve any time elements that might have been created by the timestamp script
        const timeElements = element.querySelectorAll('time.u-dt, time.when, .u-dt, .when');
        
        timeElements.forEach(timeEl => {
            // Add a data attribute to mark as preserved
            timeEl.setAttribute('data-timestamp-preserved', 'true');
            
            // Ensure visibility
            if (timeEl.style.visibility !== 'visible') {
                timeEl.style.visibility = 'visible';
            }
        });
    }

    #restoreTimestampElements(postElement) {
        // Find any time elements that might have lost their event listeners
        const timeElements = postElement.querySelectorAll('time.u-dt, time.when, .u-dt, .when');
        
        timeElements.forEach(timeEl => {
            // Re-apply visibility if needed
            if (timeEl.style.visibility !== 'visible') {
                timeEl.style.visibility = 'visible';
            }
            
            // Ensure proper attributes
            if (!timeEl.hasAttribute('dir')) {
                timeEl.setAttribute('dir', 'auto');
            }
            
            // Re-apply title if missing
            if (!timeEl.hasAttribute('title') && timeEl.hasAttribute('datetime')) {
                try {
                    ...
                    timeEl.setAttribute('title', formatted);
                } catch (e) {
                    // Ignore errors
                }
            }
            
            // Ensure timestamp script marker is present
            if (!timeEl.classList.contains('timestamp-processed')) {
                timeEl.classList.add('timestamp-processed');
            }
        });
    }
