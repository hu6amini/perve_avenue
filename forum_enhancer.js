...
// enhanced accessibility, modern code blocks, Moment.js timestamps, and emoji replacement
class PostModernizer {
    ...
    #init() {
        try {
            ...
            this.#setupEmojiReplacementObserver();
    ...
    #setupEmojiReplacementObserver() {
        // First, process existing emojis
        this.#processAllEmojiContainers();
        
        // Then set up observer for new emojis
        this.#emojiObserverId = globalThis.forumObserver.register({
            id: 'emoji-replacement-system',
            callback: (node) => this.#handleEmojiMutations(node),
            selector: '.st-emoji-container, .st-emoji-preview, .st-emoji-counter',
            priority: 'high'
        });
    }

    #handleEmojiMutations(node) {
        if (!node) return;
        
        // Check if this node or its children are emoji containers
        if (node.matches('.st-emoji-container') || 
            node.closest('.st-emoji-container') || 
            node.querySelector('.st-emoji-container')) {
            
            // Process the container
            const container = node.matches('.st-emoji-container') ? node : 
                            node.closest('.st-emoji-container') || 
                            node.querySelector('.st-emoji-container');
        ...
        // Also process counters directly
        if (node.matches('.st-emoji-counter') || node.querySelector('.st-emoji-counter')) {
        ...
        if (node.matches('.st-emoji-preview') || node.querySelector('.st-emoji-preview')) {
            const container = node.closest('.st-emoji-container');
            if (container) {
                this.#processEmojiContainer(container);
            }
    ...
    #processEmojiContainer(container) {
        ...
        const emojiCounter = container.querySelector('.st-emoji-counter');
        const emojiPreview = container.querySelector('.st-emoji-preview');
        
        if (!emojiPreview) return;
        
        const hasCounter = emojiCounter && (
            (emojiCounter.dataset && emojiCounter.dataset.count && emojiCounter.dataset.count !== '0') ||
            (emojiCounter.textContent && emojiCounter.textContent.trim && 
             emojiCounter.textContent.trim() !== '0' && 
             !isNaN(emojiCounter.textContent.trim()))
        );
        
        // Find the img element inside the preview
        const imgElement = emojiPreview.querySelector('img');
        
        if (hasCounter) {
            // If there's a counter, ensure we have the image
            if (!imgElement && emojiPreview.querySelector('i.fa-regular.fa-face-smile')) {
                // Replace Font Awesome icon with original image
                const originalImg = document.createElement('img');
                originalImg.src = 'https://cdn.jsdelivr.net/emojione/assets/svg/1f600.svg';
                originalImg.alt = 'emoji preview';
                originalImg.loading = 'lazy';
                originalImg.decoding = 'async';
                originalImg.style.aspectRatio = '20 / 20';
                originalImg.style.display = 'inline-block';
                originalImg.style.verticalAlign = 'text-bottom';
                originalImg.width = 20;
                originalImg.height = 20;
                
                // Remove the Font Awesome icon
                const faIcon = emojiPreview.querySelector('i.fa-regular.fa-face-smile');
                if (faIcon) {
                    faIcon.remove();
                }
                
                emojiPreview.appendChild(originalImg);
            }
        } else {
    ...
    #checkInitialActiveStates() {
        ...
        emojiContainers.forEach(container => {
            this.#updateEmojiContainerActiveState(container);
            this.#processEmojiContainer(container); // Also process emoji replacement
        });
    ...
    #updateAllEmojiActiveStates() {
        ...
        emojiContainers.forEach(container => {
            this.#updateEmojiContainerActiveState(container);
            this.#processEmojiContainer(container); // Process emoji replacement when active state changes
        });
    }
    ...
    #transformPostElements() {
        ...
        posts.forEach((post, index) => {
            ...
            // Process any emojis in this post after transformation
            post.querySelectorAll('.st-emoji-container').forEach(container => {
                this.#processEmojiContainer(container);
            });
        ...
        if (miniButtons || stEmoji) {
            ...
            if (stEmoji) {
                ...
                if (emojiContainer) {
                    ...
                    this.#processEmojiContainer(emojiContainer); // Process emoji replacement
                }
