...
// Video iframe replacement functionality
function VideoIframeReplacer() {
    this.processedKey = 'data-lite-processed';
    this.selectors = '.post .color iframe, #loading .color iframe';
    
    // Initialize
    this.init();
}

VideoIframeReplacer.prototype = {
    init: function() {
        if (!globalThis.forumObserver) {
            console.error('ForumCoreObserver not available');
            return;
        }
        
        // Register with forum observer
        this.callbackId = globalThis.forumObserver.register({
            id: 'video-iframe-replacer',
            callback: this.processNode.bind(this),
            selector: this.selectors,
            priority: 'high',
            dependencies: ['iframe', 'video', 'youtube', 'vimeo']
        });
        
        console.log('VideoIframeReplacer registered with ID: ' + this.callbackId);
        
        // Also process existing iframes immediately
        this.scanExistingIframes();
    },
    
    scanExistingIframes: function() {
        var iframes = document.querySelectorAll(this.selectors);
        for (var i = 0; i < iframes.length; i++) {
            this.processIframe(iframes[i]);
        }
    },
    
    processNode: function(node) {
        // Check if this is an iframe node or contains iframes
    ...
    processIframe: function(iframe) {
        // Skip if already processed
        if (iframe.hasAttribute(this.processedKey)) return;
        ...
        try {
            ...
            if (element && videoId) {
                // Copy attributes
                this.copyAttributes(iframe, element);
                
                // Mark as processed
                iframe.setAttribute(this.processedKey, 'true');
                
                // Replace
                iframe.parentNode.replaceChild(element, iframe);
                
                console.log('Replaced ' + (src.includes('youtube') ? 'YouTube' : 'Vimeo') + ' iframe (ID: ' + videoId + ')');
            }
        } catch (error) {
    ...
    // Public method to manually trigger replacement
    ...
    // Cleanup
    destroy: function() {
        if (this.callbackId && globalThis.forumObserver) {
            globalThis.forumObserver.unregister(this.callbackId);
        }
    }
};

// Initialize when DOM is ready and video libraries are loaded
function initVideoIframeReplacer() {
    // Wait for DOM and custom elements to be ready
    function waitForRequirements() {
        // Check if DOM is ready
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', waitForRequirements);
            return;
        }
        ...
        function checkCustomElements() {
            ...
            if (youtubeReady && vimeoReady) {
                // Both libraries are ready
                if (!globalThis.videoIframeReplacer) {
                    globalThis.videoIframeReplacer = new VideoIframeReplacer();
                }
            } else if (checkCount < maxChecks) {
                checkCount++;
                setTimeout(checkCustomElements, checkInterval);
            } else {
                console.warn('Video custom elements not loaded after timeout');
                // Initialize anyway for fallback processing
                if (!globalThis.videoIframeReplacer) {
                    globalThis.videoIframeReplacer = new VideoIframeReplacer();
                }
            }
...
if (typeof document !== 'undefined') {
    ...
    if ('requestIdleCallback' in window) {
        requestIdleCallback(initVideoIframeReplacer, { timeout: 2000 });
    } else {
        // Fallback for older browsers
        setTimeout(initVideoIframeReplacer, 100);
    }
}

// Export for manual control
if (typeof globalThis !== 'undefined') {
    globalThis.initVideoIframeReplacer = initVideoIframeReplacer;
    globalThis.VideoIframeReplacer = VideoIframeReplacer;
}

// Minimal fallback for direct usage
if (typeof module !== 'undefined' && module.exports) {
    module.exports = {
        VideoIframeReplacer: VideoIframeReplacer,
        initVideoIframeReplacer: initVideoIframeReplacer
    };
}
