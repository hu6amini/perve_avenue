...
// enhanced accessibility, modern code blocks, Moment.js timestamps, and Universal Emoji Replacement
class PostModernizer {
    ...
    #counterObserverId = null;
    #counterMutationObserver = null;
    #retryTimeoutId = null;
    #maxRetries = 10;
    #retryCount = 0;
    ...
    #init() {
        try {
            ...
            console.log('✅ Post Modernizer with Universal Emoji Replacement initialized');
        } catch (error) {
    ...
    // ==============================
    // UNIVERSAL EMOJI REPLACEMENT SYSTEM
    // ==============================
        ...
        console.log('😊 Universal Emoji Replacement System initialized');
    }

    #setupEmojiObservers() {
        if (globalThis.forumObserver) {
            this.#emojiObserverId = globalThis.forumObserver.register({
                id: 'post-modernizer-emoji-replacement',
                callback: (node) => this.#handleEmojiContainerChange(node),
                selector: '.st-emoji-container',
                priority: 'high'
            });

            this.#counterObserverId = globalThis.forumObserver.register({
                id: 'post-modernizer-counter-monitor',
                callback: (node) => this.#handleCounterChange(node),
                selector: '.st-emoji-counter',
                priority: 'critical'
            });
        }
    }

    #setupCounterChangeMonitoring() {
        this.#counterMutationObserver = new MutationObserver((mutations) => {
    ...
    #processAllEmojiReplacements() {
        const containers = document.querySelectorAll('.st-emoji-container');
        containers.forEach(container => {
            this.#processEmojiContainer(container);
        });
        console.log('🔍 Processed ' + containers.length + ' emoji containers');
    }

    #processEmojiContainer(container) {
        if (!container || !container.isConnected) return;
        
        const hasCounter = this.#hasValidCounter(container);
    ...
    #hasValidCounter(container) {
        ...
        const text = counter.textContent ? counter.textContent.trim() : '';
        const hasValidText = text && text !== '0' && !isNaN(text);
        
        const dataCount = counter.dataset ? counter.dataset.count : null;
        const hasValidDataCount = dataCount && dataCount !== '0';
        
        return hasValidText || hasValidDataCount;
    }

    #replaceEmojiWithFontAwesome(container) {
        const preview = container.querySelector('.st-emoji-preview');
        const img = preview ? preview.querySelector('img') : null;
        
        if (!preview || !img) return;
        
        if (preview.querySelector('i.fa-face-smile')) return;
        ...
        container.dataset.originalEmoji = JSON.stringify(originalImage);
        
        const faIcon = document.createElement('i');
        faIcon.className = 'fa-regular fa-face-smile';
        faIcon.setAttribute('aria-hidden', 'true');
        faIcon.setAttribute('role', 'img');
        faIcon.setAttribute('aria-label', 'Smiley face');
        
        const imgHeight = img.height || 20;
        const imgWidth = img.width || 20;
        
        faIcon.style.cssText = 'font-size: ' + imgHeight + 'px;' +
                              'width: ' + imgWidth + 'px;' +
                              'height: ' + imgHeight + 'px;' +
        ...
        faIcon.addEventListener('mouseenter', function() {
            this.style.transform = 'scale(1.1)';
            this.style.color = '#ff9900';
        });
        
        faIcon.addEventListener('mouseleave', function() {
            this.style.transform = 'scale(1)';
            this.style.color = '#ffcc00';
        });
        
        img.remove();
        preview.appendChild(faIcon);
        container.classList.add('emoji-replaced');
        
        console.log('✅ Replaced emoji with Font Awesome icon');
    }

    #removeFontAwesomeIfPresent(container) {
    ...
    #restoreEmojiImage(container) {
        this.#removeFontAwesomeIfPresent(container);
    }

    #cleanupEmojiSystem() {
        if (this.#emojiObserverId && globalThis.forumObserver) {
            globalThis.forumObserver.unregister(this.#emojiObserverId);
        }
        
        if (this.#counterObserverId && globalThis.forumObserver) {
            globalThis.forumObserver.unregister(this.#counterObserverId);
        }
