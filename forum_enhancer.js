...
// Modern Forum Likes/Dislikes Modal System
// Replaces the native popup with a modern emerald-themed modal
class ModernLikesModal {
    modalObserverId = null;
    themeListenerId = null;
    activeModal = null;
    currentTheme = 'dark';
    ...
    init() {
        ...
        // Process existing modals
        this.processExistingModals();
        
        console.log('🎭 Modern Likes Modal initialized');
    }
    ...
    detectTheme() {
        ...
        this.currentTheme = themeAttr || (prefersDark ? 'dark' : 'light');
    }
    ...
    registerObserver() {
        if (!globalThis.forumObserver) {
            setTimeout(() => this.registerObserver(), 100);
            return;
        }
        
        this.modalObserverId = globalThis.forumObserver.register({
            id: 'modern-likes-modal',
            callback: (node) => this.handleModalMutations(node),
            selector: '.popup.pop_points, #overlay.pop_points, .points_up, .points_down',
            priority: 'critical',
    ...
    handleModalMutations(node) {
        ...
        // Check if a modal is being shown
        const isModal = node.matches('.popup.pop_points') || 
                       node.matches('#overlay.pop_points') ||
                       (node.id === 'overlay' && node.classList.contains('pop_points'));
        
        if (isModal) {
            // Hide the original modal immediately
            this.hideOriginalModal(node);
            
            // Extract data and show modern modal
            this.extractAndShowModernModal(node);
            return;
        }
        
        // Check for points_up/down clicks that might trigger modal
        if (node.matches('.points_up') || node.matches('.points_down')) {
            // The modal might appear after a delay, so we need to watch for it
            this.watchForModalAppearance();
        }
        
        // Also check children
        if (node.querySelector) {
            const childModal = node.querySelector('.popup.pop_points, #overlay.pop_points');
            if (childModal) {
                this.hideOriginalModal(childModal);
                this.extractAndShowModernModal(childModal);
            }
            
            const pointsElements = node.querySelectorAll('.points_up, .points_down');
    ...
    hideOriginalModal(modalElement) {
        ...
        const overlay = document.querySelector('#overlay_background, .overlay-bg');
        if (overlay) {
            overlay.style.display = 'none';
        }
        
        // Prevent any further show attempts
        modalElement.setAttribute('data-modern-replaced', 'true');
    }
    
    extractAndShowModernModal(modalElement) {
        if (!modalElement) return;
        
        // Extract likes and dislikes data
        const likesData = this.extractLikesData(modalElement);
        const dislikesData = this.extractDislikesData(modalElement);
        
        // Get post ID from the modal context
        const postId = this.extractPostId(modalElement);
        
        // Show modern modal
        this.showModernModal(likesData, dislikesData, postId);
    }
    
    extractLikesData(modalElement) {
        const likes = {
            count: 0,
            users: []
        };
        ...
        // Find likes section
        const pointsUpSpan = modalElement.querySelector('.points_up');
        if (!pointsUpSpan) return likes;
        
        // Get likes count from icon or text
        const likesText = pointsUpSpan.nextSibling ? pointsUpSpan.nextSibling.textContent || '' : '';
        const countMatch = likesText.match(/(\d+)/);
        likes.count = countMatch ? parseInt(countMatch[1]) : 0;
        
        // Get likes users list (next ol after likes section)
        let nextElement = pointsUpSpan.nextElementSibling;
        while (nextElement && !nextElement.matches('ol.users')) {
            nextElement = nextElement.nextElementSibling;
        }
        
        if (nextElement && nextElement.matches('ol.users')) {
            likes.users = this.extractUsersFromList(nextElement, 'like');
        }
        
        return likes;
    }
    
    extractDislikesData(modalElement) {
        const dislikes = {
            count: 0,
            users: []
        };
        
        // Find dislikes section
        const pointsDownSpan = modalElement.querySelector('.points_down');
        if (!pointsDownSpan) return dislikes;
        
        // Get dislikes count from icon or text
        const dislikesText = pointsDownSpan.nextSibling ? pointsDownSpan.nextSibling.textContent || '' : '';
        const countMatch = dislikesText.match(/(\d+)/);
        dislikes.count = countMatch ? parseInt(countMatch[1]) : 0;
        
        // Get dislikes users list (next ol after dislikes section)
        let nextElement = pointsDownSpan.nextElementSibling;
        while (nextElement && !nextElement.matches('ol.users')) {
            nextElement = nextElement.nextElementSibling;
        }
        
        if (nextElement && nextElement.matches('ol.users')) {
            dislikes.users = this.extractUsersFromList(nextElement, 'dislike');
        }
        
        return dislikes;
    }
    
    extractUsersFromList(listElement, type) {
        const users = [];
        if (!listElement) return users;
        
        listElement.querySelectorAll('li').forEach((li) => {
            const user = this.extractUserFromListItem(li, type);
            if (user) {
                users.push(user);
            }
        });
    ...
    extractUserFromListItem(liElement, type) {
        const user = {
            id: '',
            username: '',
            avatar: '',
        ...
        // Extract user link
        const userLink = liElement.querySelector('a[href*="Profile"]');
        if (userLink) {
            user.username = userLink.textContent.trim();
            user.profileUrl = userLink.getAttribute('href') || '#';
            
            // Extract user ID from href or class
            const midMatch = user.profileUrl.match(/MID=(\d+)/);
            if (midMatch) {
                user.id = midMatch[1];
            }
            
            // Check for user ID in class
            const userClass = userLink.className;
            const userIdMatch = userClass.match(/user(\d+)/);
            if (userIdMatch) {
        ...
        // Extract avatar
        const avatarImg = liElement.querySelector('img.forum-likes-avatar, .forum-avatar-container img');
        if (avatarImg) {
            user.avatar = avatarImg.getAttribute('src') || '';
        } else {
            // Try to find avatar in the has-forum-avatar span
            const avatarSpan = liElement.querySelector('.has-forum-avatar');
            if (avatarSpan) {
                const spanAvatar = avatarSpan.querySelector('img');
                if (spanAvatar) {
                    user.avatar = spanAvatar.getAttribute('src') || '';
                }
            }
        }
        
        // Extract undo information (but we won't show the bullet delete)
        const bulletDelete = liElement.querySelector('.bullet_delete');
        if (bulletDelete) {
            user.canUndo = true;
            user.undoAction = bulletDelete.getAttribute('onclick') || '';
        }
        
        return user;
    }
    
    extractPostId(modalElement) {
        // Try to find post ID from modal context
        let postId = '';
        
        // Check for post ID in nearby elements
        const modalParent = modalElement.closest('.post');
        if (modalParent && modalParent.id) {
            const idMatch = modalParent.id.match(/\d+/);
            if (idMatch) {
                postId = idMatch[0];
            }
        }
        
        // Check for post ID in undo actions
        const undoScript = modalElement.querySelector('.bullet_delete[onclick*="points"]');
        if (undoScript) {
            const onclick = undoScript.getAttribute('onclick') || '';
            const pointsMatch = onclick.match(/points(\d+)/);
            if (pointsMatch) {
                postId = pointsMatch[1];
            }
        }
    ...
    showModernModal(likesData, dislikesData, postId) {
        // Close any existing modal first
        this.closeModal();
        
        // Don't show modal if there are no likes or dislikes
        if ((!likesData || likesData.users.length === 0) && 
            (!dislikesData || dislikesData.users.length === 0)) {
            return;
        }
        
        // Create modal structure
        const modal = this.createModalElement(likesData, dislikesData, postId);
    ...
    createModalElement(likesData, dislikesData, postId) {
        const modal = document.createElement('div');
        modal.className = 'modern-likes-modal';
        modal.setAttribute('data-theme', this.currentTheme);
        modal.setAttribute('role', 'dialog');
        modal.setAttribute('aria-modal', 'true');
        modal.setAttribute('aria-label', 'Post reactions');
        
        // Build modal content
        let html = '<div class="modern-likes-modal__backdrop"></div>' +
                   '<div class="modern-likes-modal__container" role="document">' +
                   '<div class="modern-likes-modal__header">' +
                   '<h2 class="modern-likes-modal__title">' +
                   '<i class="fa-regular fa-thumbs-up" aria-hidden="true"></i>' +
                   ' Reactions' +
                   '</h2>' +
                   '<button class="modern-likes-modal__close" type="button" aria-label="Close modal">' +
                   '<i class="fa-regular fa-xmark" aria-hidden="true"></i>' +
                   '</button>' +
                   '</div>' +
                   '<div class="modern-likes-modal__content">';
        
        // Add likes section if there are likes
        if (likesData && likesData.users.length > 0) {
            html += this.createReactionSection('likes', likesData);
        }
        
        // Add dislikes section if there are dislikes
        if (dislikesData && dislikesData.users.length > 0) {
            html += this.createReactionSection('dislikes', dislikesData);
        }
        
        html += '</div>' +
                '<div class="modern-likes-modal__footer">' +
                '<span class="modern-likes-modal__post-info">Post #' + this.escapeHtml(postId || '') + '</span>' +
                '</div>' +
                '</div>';
    ...
    createReactionSection(type, data) {
        const isLikes = type === 'likes';
        const iconClass = isLikes ? 'fa-thumbs-up' : 'fa-thumbs-down';
        const title = isLikes ? 'Likes' : 'Dislikes';
        const count = data.users.length;
        const colorClass = isLikes ? 'success-color' : 'danger-color';
        
        let html = '<div class="modern-likes-modal__section modern-likes-modal__section--' + type + '">' +
                   '<div class="modern-likes-modal__section-header">' +
                   '<div class="modern-likes-modal__section-title">' +
                   '<i class="fa-regular ' + iconClass + ' ' + colorClass + '" aria-hidden="true"></i>' +
                   '<span>' + this.escapeHtml(title) + '</span>' +
                   '<span class="modern-likes-modal__count">' + count + '</span>' +
                   '</div>' +
                   '</div>' +
                   '<div class="modern-likes-modal__users">';
        
        // Add each user
        data.users.forEach((user, index) => {
            html += this.createUserElement(user, isLikes, index);
        });
    ...
    createUserElement(user, isLike, index) {
        const reactionIcon = isLike ? 'fa-thumbs-up' : 'fa-thumbs-down';
        const reactionColor = isLike ? 'success' : 'danger';
        
        let html = '<div class="modern-likes-modal__user" data-user-id="' + this.escapeHtml(user.id) + '" tabindex="0">';
        
        // Avatar
        if (user.avatar) {
            html += '<div class="modern-likes-modal__user-avatar">' +
                    '<img src="' + this.escapeHtml(user.avatar) + '" alt="' + this.escapeHtml(user.username) + '" loading="lazy" decoding="async" width="40" height="40">' +
                    '</div>';
        } else {
            // Default avatar
            html += '<div class="modern-likes-modal__user-avatar modern-likes-modal__user-avatar--default">' +
                    '<i class="fa-regular fa-user" aria-hidden="true"></i>' +
                    '</div>';
        }
        
        // User info
        html += '<div class="modern-likes-modal__user-info">' +
                '<a href="' + this.escapeHtml(user.profileUrl) + '" class="modern-likes-modal__user-name" target="_self" rel="nofollow">' +
                this.escapeHtml(user.username) +
                '</a>';
        
        // Reaction icon next to username
        html += '<span class="modern-likes-modal__user-reaction ' + reactionColor + '-reaction" title="' + (isLike ? 'Liked' : 'Disliked') + '">' +
                '<i class="fa-regular ' + reactionIcon + '" aria-hidden="true"></i>' +
                '</span>';
    ...
    addModalEventListeners(modal) {
        // Close button
        const closeBtn = modal.querySelector('.modern-likes-modal__close');
        if (closeBtn) {
        ...
        // Backdrop click
        const backdrop = modal.querySelector('.modern-likes-modal__backdrop');
        if (backdrop) {
        ...
        // User click - open profile in new tab
        modal.querySelectorAll('.modern-likes-modal__user-name').forEach(link => {
        ...
        // User container click - also open profile
        modal.querySelectorAll('.modern-likes-modal__user').forEach(userDiv => {
            userDiv.addEventListener('click', (e) => {
                if (e.target.closest('.modern-likes-modal__user-name')) {
                    return; // Let the link handler deal with it
                }
                
                const link = userDiv.querySelector('.modern-likes-modal__user-name');
                if (link) {
                    const href = link.getAttribute('href');
                    if (href && href !== '#') {
                        window.open(href, '_blank', 'noopener,noreferrer');
    ...
    addEscapeListener() {
        ...
        if (this.activeModal) {
            this.activeModal._escapeHandler = escapeHandler;
        }
    }
    ...
    closeModal() {
        ...
        const escapeHandler = this.activeModal._escapeHandler;
        if (escapeHandler) {
    ...
    updateModalTheme() {
        ...
        this.activeModal.setAttribute('data-theme', this.currentTheme);
    }
    
    watchForModalAppearance() {
        // Watch for modal appearance for the next 2 seconds
        let watchInterval = null;
        let watchCount = 0;
        const maxWatchCount = 20; // 2 seconds at 100ms intervals
        
        const checkForModal = () => {
            const modal = document.querySelector('.popup.pop_points, #overlay.pop_points');
            
            if (modal && !modal.hasAttribute('data-modern-replaced')) {
                this.hideOriginalModal(modal);
                this.extractAndShowModernModal(modal);
        ...
        watchInterval = setInterval(checkForModal, 100);
    }
    
    processExistingModals() {
        // Check if there's already a modal on the page
        const existingModal = document.querySelector('.popup.pop_points, #overlay.pop_points');
        if (existingModal && !existingModal.hasAttribute('data-modern-replaced')) {
            this.hideOriginalModal(existingModal);
            this.extractAndShowModernModal(existingModal);
        }
    }
    ...
    destroy() {
        // Unregister observer
        if (this.modalObserverId && globalThis.forumObserver) {
            globalThis.forumObserver.unregister(this.modalObserverId);
        }
        
        // Remove theme listener
        if (this.themeListenerId) {
            window.removeEventListener('themechange', this.themeListenerId);
        }
        
        // Close active modal
        this.closeModal();
        
        console.log('Modern Likes Modal destroyed');
    }
}

// CSS injection for the modern modal
(function injectModalStyles() {
    const styleId = 'modern-likes-modal-styles';
    ...
    styles.textContent = `
        ...
        .modern-likes-modal.active {
            opacity: 1;
            pointer-events: all;
        }
        
        .modern-likes-modal__backdrop {
            position: absolute;
            top: 0;
            left: 0;
            width: 100%;
        ...
        .modern-likes-modal.active .modern-likes-modal__container {
            transform: translateY(0);
        }
        
        .modern-likes-modal__header {
            display: flex;
            align-items: center;
        ...
        .modern-likes-modal__title i {
            color: var(--primary-color);
        }
        
        .modern-likes-modal__close {
        ...
        .modern-likes-modal__close:hover {
            background: var(--hover-color);
            color: var(--text-primary);
            transform: rotate(90deg);
        }
        
        .modern-likes-modal__close i {
            font-size: 1.25rem;
        }
        
        .modern-likes-modal__content {
            flex: 1;
            overflow-y: auto;
            padding: var(--space-lg);
        }
        
        .modern-likes-modal__section {
            margin-bottom: var(--space-xl);
        }
        
        .modern-likes-modal__section:last-child {
            margin-bottom: 0;
        }
        
        .modern-likes-modal__section-header {
            margin-bottom: var(--space-md);
        }
        
        .modern-likes-modal__section-title {
            display: flex;
            align-items: center;
            gap: var(--space-sm);
        ...
        .modern-likes-modal__section-title i {
            font-size: 1.1em;
        }
        
        .modern-likes-modal__section-title i.success-color {
            color: var(--success-color);
        }
        
        .modern-likes-modal__section-title i.danger-color {
            color: var(--danger-color);
        }
        
        .modern-likes-modal__count {
            margin-left: auto;
            background: var(--bg-color);
            border: 1px solid var(--border-color);
            border-radius: 12px;
        ...
        .modern-likes-modal__users {
            display: flex;
            flex-direction: column;
            gap: var(--space-sm);
        }
        
        .modern-likes-modal__user {
            display: flex;
            align-items: center;
            gap: var(--space-md);
        ...
        .modern-likes-modal__user:hover,
        .modern-likes-modal__user:focus {
            background: var(--hover-color);
        ...
        .modern-likes-modal__user-avatar {
            width: 40px;
            height: 40px;
            flex-shrink: 0;
        }
        
        .modern-likes-modal__user-avatar img {
            width: 100%;
            height: 100%;
            border-radius: 50%;
            object-fit: cover;
        ...
        .modern-likes-modal__user-avatar--default {
        ...
        .modern-likes-modal__user-avatar--default i {
            font-size: 1.25rem;
        }
        
        .modern-likes-modal__user-info {
            flex: 1;
            display: flex;
            align-items: center;
            gap: var(--space-sm);
        ...
        .modern-likes-modal__user-name:hover {
            color: var(--primary-color);
            text-decoration: underline;
        }
        
        .modern-likes-modal__user-reaction {
            display: flex;
            align-items: center;
            justify-content: center;
            width: 24px;
        ...
        .modern-likes-modal__user-reaction.success-reaction {
            background: rgba(16, 185, 129, 0.15);
            color: var(--success-color);
        }
        
        .modern-likes-modal__user-reaction.danger-reaction {
            background: rgba(220, 38, 38, 0.15);
            color: var(--danger-color);
        }
        
        .modern-likes-modal__user-reaction i {
            font-size: 0.875rem;
        }
        
        .modern-likes-modal__footer {
        ...
        .modern-likes-modal[data-theme="light"] .modern-likes-modal__backdrop {
            background: rgba(0, 0, 0, 0.3);
        }
        
        .modern-likes-modal[data-theme="light"] .modern-likes-modal__user-avatar--default {
        ...
        /* Animation for new modals */
        @keyframes modern-modal-appear {
            from {
                opacity: 0;
        ...
        .modern-likes-modal__container {
            animation: modern-modal-appear 0.3s cubic-bezier(0.4, 0, 0.2, 1);
        }
    `;
...
// Initialize the modern modal system
(function initModernLikesModal() {
    // Only initialize on topic pages where likes/dislikes exist
    const bodyId = document.body.id;
    const shouldInitialize = bodyId === 'topic' || bodyId === 'blog';
    ...
    const init = function() {
        try {
            globalThis.modernLikesModal = new ModernLikesModal();
            console.log('🎭 Modern Likes Modal System ready');
        } catch (error) {
            console.error('Failed to initialize Modern Likes Modal:', error);
            
            // Retry once
            setTimeout(function() {
                if (!globalThis.modernLikesModal) {
                    try {
                        globalThis.modernLikesModal = new ModernLikesModal();
                    } catch (retryError) {
                        console.error('Modern Likes Modal failed on retry:', retryError);
                    }
                }
            }, 500);
        }
...
globalThis.addEventListener('pagehide', function() {
    if (globalThis.modernLikesModal && typeof globalThis.modernLikesModal.destroy === 'function') {
        globalThis.modernLikesModal.destroy();
    }
});
