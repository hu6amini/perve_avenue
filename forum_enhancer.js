...
// WITH THEME SWITCHER INTEGRATION AND CONSOLIDATED TOOLS MENU
class EnhancedMenuModernizer { 
 ...
 this.#initializeTheme(); // Initialize theme system 
 ...
 // Instead of showing all icon menus, create a single consolidated "Tools" menu
 return [this.#createToolsMenu(menus)];
 } 
 
 #createToolsMenu(menuElements) {
   // Group items by type
   const groups = {
     community: { title: 'Community', items: [] },
     resources: { title: 'Resources', items: [] },
     feeds: { title: 'Feeds', items: [] },
     help: { title: 'Help & Support', items: [] }
   };
   ...
   menuElements.forEach((menu) => {
     ...
     const dropdownItems = menu.querySelectorAll('ul li a');
     
     // Categorize based on content
     if (text.includes('Members') || text.includes('Users')) {
       groups.community.items.push({ text, href, items: dropdownItems });
     } else if (text.includes('RSS') || text.includes('Feed')) {
       groups.feeds.items.push({ text, href, items: dropdownItems });
     } else if (text.includes('Help')) {
       groups.help.items.push({ text, href, items: dropdownItems });
     } else {
       groups.resources.items.push({ text, href, items: dropdownItems });
     }
   });
 
   // Build the consolidated tools menu HTML
   let dropdownHTML = '';
   
   // Add theme switcher at top
   dropdownHTML += '<div class="tools-group theme-section">' +
   ...
   for (const [groupKey, group] of Object.entries(groups)) {
     ...
     dropdownHTML += '<div class="tools-group">' +
       '<div class="tools-group-title">' + group.title + '</div>';
 
     group.items.forEach(item => {
       const icon = this.#getIconForText(item.text);
       
       if (item.items && item.items.length > 0) {
         // Has sub-items
         const firstItem = item.items[0];
         const mainHref = firstItem ? firstItem.getAttribute('href') : item.href;
         
         dropdownHTML += '<a href="' + this.#escapeHtml(mainHref) + '" class="dropdown-item with-icon">' +
         ...
         item.items.forEach((subItem, i) => {
           if (i < 3) { // Limit to 3 items
             ...
             dropdownHTML += '<a href="' + this.#escapeHtml(subItem.getAttribute('href') || '#') + 
               '" class="dropdown-item with-icon sub-item">' +
               '<i class="fa-regular ' + this.#getIconForText(subText) + '" aria-hidden="true"></i>' +
               '<span>' + this.#escapeHtml(subText) + '</span>' +
               '</a>';
           }
         });
       } else {
         // Simple link
         dropdownHTML += '<a href="' + this.#escapeHtml(item.href) + '" class="dropdown-item with-icon">' +
           '<i class="fa-regular ' + icon + '" aria-hidden="true"></i>' +
           '<span>' + this.#escapeHtml(item.text) + '</span>' +
           '</a>';
       }
   ...
   return '<div class="menu-item tools-menu">' +
     '<button class="menu-trigger icon-trigger" title="Tools & Settings">' +
 ...
 #getIconForText(text) { 
   ...
   if (lowerText.includes('tools') || lowerText.includes('settings')) return 'fa-screwdriver-wrench'; 
 ...
 #addThemeSwitcherToMobileMenu() { 
   ...
   // Add event listeners to mobile theme switcher 
 ...
 #setupThemeEventListeners() { 
   ...
   // Mobile theme switcher (already handled in mobile setup) 
 } 
 ...
 #setTheme(theme) { 
   ...
   // Update active state on all theme switcher items 
 ...
 #loadTheme() { 
   ...
   if (theme === 'auto') { 
     ...
     // Auto theme should respect system preference 
     if (prefersDark) { 
 ...
 #updateActiveTheme(theme) { 
   // Update all theme switcher items 
 ...
 #buildMobileMenuHTML() { 
   ...
   // Extract right menu items as Tools menu
   if (rightUl) {
     const menus = rightUl.querySelectorAll('li.menu');
     let hasRightItems = false;
     
     // Create a consolidated Tools section
     html += '<div class="mobile-menu-item">' +
       '<button class="mobile-menu-trigger" data-dropdown="mobile-dropdown-tools">' +
       '<i class="fa-regular fa-screwdriver-wrench" aria-hidden="true"></i>' +
       '<span>Tools & Settings</span>' +
       '<i class="fa-regular fa-chevron-down" aria-hidden="true"></i>' +
       '</button>' +
       '<div class="mobile-dropdown" id="mobile-dropdown-tools">';
     ...
     menus.forEach((menu, index) => {
       ...
       html += '<div class="mobile-dropdown-item with-icon" style="font-weight: 600; color: var(--text-primary);">' +
         '<i class="fa-regular ' + this.#getIconForText(text) + '" aria-hidden="true"></i>' +
         '<span>' + this.#escapeHtml(text) + '</span>' +
         '</div>';
       ...
       dropdownItems.forEach(item => {
         ...
         html += '<a href="' + this.#escapeHtml(item.getAttribute('href') || '#') + '" class="mobile-dropdown-item with-icon sub-item">' +
     ...
     // If no right items, don't show the section
     if (!hasRightItems) {
       html = html.replace('<div class="mobile-menu-item">', '').replace('</div></div>', '');
     }
   }
