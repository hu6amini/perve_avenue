// Modern Likes Modal - FORUM OBSERVER ONLY (no fallback)
// Uses string concatenations for maximum compatibility
(function() {
    'use strict';
    ...
    // Show modern modal
    async function showModernModal(userIds) {
    ...
    function handleLegacyModal(node) {
        ...
        if (node.id === 'overlay' && 
            ...
            if (userIds.length > 0) {
                ...
                node.style.display = 'none'; // Hide original
            }
        }
    }
    ...
    if (typeof globalThis.forumObserver === 'undefined' || !globalThis.forumObserver) {
        ...
        return; // Exit silently - no fallback
    }
    ...
    // Register with ForumCoreObserver for attribute changes (style changes on the modal)
    ...
    console.log('[Modern Likes] Ready and waiting for likes modal (ForumCoreObserver only mode)');
})();
