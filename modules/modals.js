...
(function() {
    ...
    if (typeof globalThis.forumObserver === 'undefined') {
        console.log('[Modern Likes] Waiting for ForumCoreObserver...');
    ...
    function initModule() {
        ...
        // Function to handle legacy modal detection
        function handleLegacyModal(modal) {
            if (!modal || modal.id !== 'overlay' || !modal.classList.contains('pop_points')) return;
            if (modal.style.display !== 'block') return;
            if (closeCooldown || processingModal || currentCustomModal) return;
        ...
        function registerWithObserver() {
            if (typeof globalThis.forumObserver === 'undefined') {
                console.log('[Modern Likes] ForumCoreObserver not ready yet');
                return false;
            }
            
            // Register to watch for style changes on the overlay
            globalThis.forumObserver.register({
                id: 'modern-likes-modal-detector',
                selector: '#overlay.pop_points',
                priority: 'high',
                callback: function(node) {
                    // Check if the node's style display is block
            ...
            // Also watch for newly added overlay elements
            globalThis.forumObserver.register({
                id: 'modern-likes-modal-adder',
                selector: '#overlay.pop_points',
                priority: 'high',
                callback: function(node) {
                    // Only process if it's newly added and has display block
                    if (node.style.display === 'block') {
                        handleLegacyModal(node);
                    }
                }
        ...
        function init() {
            ...
            console.log('[Modern Likes] Module initialized');
        }
